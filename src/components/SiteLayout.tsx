...
const SiteLayout = () => {
  ...
  return (
    <div className="flex min-h-screen flex-col">
      <header className="bg-primary-dark text-primary-foreground shadow-[var(--shadow-soft)] sticky top-0 z-50">
        <div className="container relative flex h-20 items-center justify-between gap-3 md:h-28">
          <Link to="/" className="flex min-w-0 items-center gap-2 text-lg font-bold md:gap-3 md:text-2xl">
            <img
              src={logo}
              alt="Logo EcoTech - reciclagem de eletrônicos"
              className="h-14 w-14 shrink-0 object-contain drop-shadow-md md:h-20 md:w-20"
            />
            <span className="truncate bg-gradient-to-r from-primary-light to-white bg-clip-text text-transparent">EcoTech</span>
          </Link>
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `text-sm font-medium transition-colors hover:text-primary-light ${
                        isActive ? "text-primary-light" : "text-primary-foreground"
                      }`
                    }
                  >
                    {item.label}
          ...
          <button
            ...
            aria-expanded={open}
            onClick={() => setOpen((v) => !v)}
            className="md:hidden inline-flex items-center justify-center rounded-md p-2 transition-colors hover:bg-white/10"
          >
        ...
        {/* Mobile nav panel */}
        {open && (
          <nav className="md:hidden border-t border-white/10 bg-primary-dark">
      ...
      <footer className="bg-primary-dark text-primary-foreground mt-16">
        <div className="container py-10 text-center">
          ...
          <div className="my-5 flex justify-center gap-5">
            ...
            <a href="#" aria-label="Instagram" className="transition-transform hover:scale-110 hover:text-primary-light">
              <Instagram className="h-5 w-5" />
            </a>
            <a href="#" aria-label="Twitter" className="transition-transform hover:scale-110 hover:text-primary-light">
              <Twitter className="h-5 w-5" />
            </a>
          </div>
          <div className="flex flex-wrap justify-center gap-x-4 gap-y-2 text-sm opacity-90">
            <a href="#" className="hover:text-primary-light hover:underline">Termos de Uso</a>
            <a href="#" className="hover:text-primary-light hover:underline">Política de Privacidade</a>
          </div>
        </div>
      </footer>
    </div>
